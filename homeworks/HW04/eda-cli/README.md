...
### 5. `POST /quality-flags-from-csv` - получение флагов качества датасета по CSV-файлу
...
- вызывает функции из `eda_cli.core`:
  ...
  - `missing_table`,
  - `compute_quality_flags`;
- возвращает флаги с оценкой датасета;
...
### 6. `POST /head?n={}` - получение первых строчек из датасета в формате JSON

Эндпоинт принимает CSV файл и query-параметр n - количество первых строчек для вывода.
