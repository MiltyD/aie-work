...
class JsonFormatter(logging.Formatter):
    def format(self, record):
        ...
        for key, value in record.__dict__.items():
            if not key.startswith('_') and key not in log_record:
...
class HeadResponse(BaseModel):
    rows: dict[Any, Any] | None = Field(
        default=None,
        description="Первые строчки датасета",
    )
...
def quality_flags_from_csv(file: UploadFile = File(...)) -> QualityFlagsResponse:
    ...
    return QualityFlagsResponse(
        flags={
            ...
            "has_constant_columns": flags_all["has_constant_columns"],
            "has_suspicious_id_duplicates": flags_all["has_suspicious_id_duplicates"]
        }
    )

@app.post("/head", response_model=HeadResponse)
def head(file: UploadFile = File(...), n: int = 5):
    if file.content_type not in ("text/csv", "application/vnd.ms-excel", "application/octet-stream"):
        # content_type от браузера может быть разным, поэтому проверка мягкая
        # но для демонстрации оставим простую ветку 400
        raise HTTPException(status_code=400, detail="Ожидается CSV-файл (content-type text/csv).")
    ...
    head_df = df.head(n)
    return HeadResponse(rows=head_df.to_dict())